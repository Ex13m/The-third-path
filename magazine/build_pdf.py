...
pages.append(text_page(4,'PARS III','ASSEMBLATIO','Его собирают не&nbsp;на&nbsp;заводе',4,'l','''
  ...
  '«Свинья от человека отличается одним: человек иногда смотрит вверх. Мы не продаём этот момент — мы его открываем. Процедура стоит доллар.»',
...
pages.append(text_page(5,'PARS IV','PROTOCOLUM','Первый запуск — на&nbsp;себе',5,'r','''
  ...
  '«Смертному объясняют, как прожить отпущенное. Мы сообщаем, что отпущенное закончилось. За сообщение денег не берём — берём за процедуру.»',
...
pages.append(f'''<section class="page">
  ...
  <div class="pull">«Я думал, что раздаю вечную жизнь. Я раздаю невозможность выйти. Проходят процедуру одинаково охотно.»</div>
  {hud_frame(7,'PARS VI')}
</section>''')

# ── 08 SOCIETAS ($1) ────────────────────────────────────────
pages.append(text_page(8,'PARS VII','SOCIETAS','Доллар — чтобы не ждать инвайт',9,'r','''
  <p>Можно было бы назвать цену квартиры. Но мы не назначаем цен — мы не продавцы. Есть только процедура, и она стоит один доллар. Не потому что дёшево, а потому что доллар есть у всех — и на этом заканчиваются отговорки.</p>
  <table class="ttx"><tr><td class="tk">ПРОЦЕДУРА</td><td>Один доллар. Разово. «В месяц» — для тех, кому нравится ритуал.</td></tr><tr><td class="tk">ЧТО ДАЁТ</td><td>Инвайт перестаёт зависеть от чужого настроения. Дальше — сам.</td></tr><tr><td class="tk">ПРОШЛИ</td><td>1 204 880. Счётчик растёт каждую секунду. Секунды здесь длинные.</td></tr><tr><td class="tk">ПЕРЕДУМАЛИ</td><td>0 человек. Подумай, что это говорит. О процедуре. Или о нас.</td></tr><tr><td class="tk">ВЫГОДА ДЛЯ НАС</td><td>Ноль. Мы честные — это написано прямо здесь.</td></tr></table>''',
  '«Я ничего не продаю. Я всего лишь убираю слово „никогда“ из твоего приглашения. Один доллар — и оно исчезает.»',
  [(107,6,'FIG.07 · PROGRAMMA CO-AUCTORIS','l'),(100,62,'PROCEDURA · $1','l')]))
...
pages.append(f'''<section class="page">
  ...
  <div class="txt tx-under-tarif">
    <div class="kicker">[PARS IX] ORDO PROCEDURARUM</div>
    <h2>Очередь к вечности — по&nbsp;процедуре</h2>
...
# ── 11 PROCEDORIUM (слот под QR — код вставляет издатель вручную) ─
payrows=[('ПРОЦЕДУРА','Один доллар. Не цена — жест. Способ сказать «я не жду приглашения, я уже иду».'),
         ('ИНВАЙТ','Приходит всем. Срок — от недели до «никогда». Процедура вычёркивает слово «никогда».'),
         ('УРОВЕНЬ 2','Что за воротами — узнают прошедшие. Спойлер обесценивает процедуру.'),
...
pages.append(f'''<section class="page">
  <div class="txt" style="position:absolute; top:16mm; left:12mm; right:12mm">
    <div class="kicker">[PARS X] PROCEDORIUM · GRADUS SECUNDUS</div>
    <h2>Процедура перехода на&nbsp;второй уровень</h2>
    <p class="lead">Никто ничего не продаёт и не покупает. Инвайт приходит сам — рано или поздно. Обычно поздно. Процедура — это способ не выяснять, насколько поздно.</p>
  </div>
...
pages.append(text_page(15,'PARS XIV','ПОКУПАТЕЛЬ ЗАВТРАШНЕГО ДНЯ','EMPTOR CRASTINUS',15,'l','''
  ...
  <p>Отдел лора его обожает: самый лояльный участник из всех. Пройдёт процедуру сразу на сто долларов — через двадцать лет. Посмертно. Завещание уже готово и лежит у нашего кибер-агента. Участник его ещё не писал — но кибер-агент никуда не торопится. В этом их главное различие.</p>
...
pages.append(text_page(16,'PARS XV','FINIS · БУДУЩЕЕ, КОТОРОЕ ОБЯЗАНО СЛУЧИТЬСЯ','FUTURUM DEBITUM',8,'l','''
  ...
  <p>Процедура входа — один доллар. Не потому что дёшево, а потому что доллар есть у всех. Отговорки конструкцией не предусмотрены.</p>
